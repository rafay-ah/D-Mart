   ...
   $count = 0;
    ...
    @if ($message = Session::get('Success'))
        ...
        <div id="" class="alert alert-success alert-block" >
            <strong> Product Have Been Added Successfully Removed From Cart  </strong>
        </div>
    @endif
    <h1 style="margin: auto; padding: 10px; background-color: orange; border-radius: 20%; margin-bottom: 30px; color: white">Cart List</h1>
    <table class="table table-hover">
        ...
        <tr>
            ...
            <td>{{$product['price']}}</td>
            <td><a href="{{url('/removeSingleCart/'.$product['name'])}}" style="background-color: orangered; color: white; padding: 5px; border-radius: 10%; margin: 5px">Remove</a></td>
        </tr>
{{--            $count+=$product['price'];--}}
        @endforeach
...
<br>
    <div class= "btn btn-dark btn-block btn-round btn-style"  style="float: right" >
    <a href="{{url('/checkout/'.$total)}}" type="submit" >
        {{ __('Checkout') }}
    </a><br></div>
